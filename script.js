...
function initProjectsPage(){
  ...
  function applyFilters(){
    ...
    if(emptyVideo){
      emptyVideo.style.display = (videoCards.length === 0) ? 'block' : (videoVisible === 0 ? 'block' : 'none');
    }

    // Persist UX project count so CV can read it.
    try{ localStorage.setItem('vanesafrz-projects-count', String(uxCards.length)); } catch(err){ /* noop */ }
  }
...
function getPortfolioProjectsCount(){
  const stored = parseInt(localStorage.getItem('vanesafrz-projects-count') || '', 10);
...
function animateNumber({ el, to = 0, prefix = '', suffix = '', duration = 1100 }){
  if(!el) return;
...
function initCvCounters(){
  ...
  if(!expEl && !projEl) return;

  const expTarget = Math.floor(yearsSinceJuly2017());
  const projTarget = getPortfolioProjectsCount();
  const trigger = expEl?.closest('.cv-header-card') || expEl || projEl;
  if(!trigger) return;

  const run = () => {
    if(expEl) animateNumber({ el: expEl, to: expTarget, prefix: '+' });
    if(projEl) animateNumber({ el: projEl, to: projTarget });
  };
